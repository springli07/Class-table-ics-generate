...
class MainWindow(QMainWindow):
    ...
    def show_context_menu(self, pos):
        ...
        if column == 0:
            ...
            if item and item.text():
                ...
                edit_action.triggered.connect(lambda: self.edit_course(row, column))
    ...
    def edit_course(self, row, column):
        """编辑指定单元格中的课程"""
        day = column - 1
        period = row
        ...
        course_data = None
        for course in self.courses:
            if course['day'] == day and course['period'] == period:
                course_data = course
                break
        ...
        # 打开编辑对话框
        dialog = CourseDialog(self, day, period, course_data)
        if dialog.exec():
            new_data = dialog.get_data()
            if not self.validate_course_data(new_data):
                return

            task_weeks = parse_weeks_input(new_data['weeks'])
            if not task_weeks:
            ...
            course_data.update({
                ...
                'location': new_data['location'].strip(),
                'weeks': task_weeks
            })
